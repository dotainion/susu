...
export const ContributionRefund = () =>{
    ...
    return(
        <div className="container">
            <div className="d-block d-sm-flex w-100 m-auto p-4 rounded-4 bg-light" style={{maxWidth: '800px'}}>
                <div className="w-100">
                    ...
                    <select ref={contributionRef} className="form-control shadow-none overflow-auto" multiple style={{height: '300px', maxHeight: '300px'}}>
                        {contributions.map((payment)=>(
                            <option onClick={()=>setSelectedContribution(payment)} key={payment.id}>{utils.date.toLocalDate(payment.attributes.data)}</option>
                        ))}
                    </select>
                </div>
